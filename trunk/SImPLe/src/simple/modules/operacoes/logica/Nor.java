...
/**
 * Classe que faz a operacao logica NOr sobre imagens
...
public class Nor {
	...
	/**
	 * Metodo nor que faz a operacao nor entre varias imagens gerando uma outra com o resultado
	 ...
	 */
	public static Image nor(List<ImageIcon> imagensIcon) throws OperacaoLogicaException{
		imageResultHeight = 0;
		imageResultWidth = 0;
		int altura, largura;
		...
		while (k < imagensIcon.size()) {
			...
			for (int i = 0; i < altura; i++) 
				for (int j = 0; j < largura; j++) 
					resultImage[PixelUtils.getIndex(i, j, imageResultWidth)] =  PixelUtils.nor(
							resultImage[PixelUtils.getIndex(i, j, imageResultWidth)],PixelUtils.getPixel(pixels, i, j, largura));
			k++;
		}
