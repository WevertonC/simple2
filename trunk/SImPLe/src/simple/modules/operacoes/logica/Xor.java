...
/**
 * Classe que faz a operacao logica XOr sobre imagens
...
public class Xor {
	...
	/**
	 * Metodo xor que faz a operacao xor em varias imagens gerando uma outra com o resultado
	 ...
	 */
	public static Image xor(List<ImageIcon> imagensIcon) throws OperacaoLogicaException{
		imageResultHeight = 0;
		imageResultWidth = 0;
		int altura, largura;
		...
		while (k < imagensIcon.size()) {
			...
			for (int i = 0; i < altura; i++) 
				for (int j = 0; j < largura; j++) 
					resultImage[PixelUtils.getIndex(i, j, imageResultWidth)] =  PixelUtils.xor(
							resultImage[PixelUtils.getIndex(i, j, imageResultWidth)],PixelUtils.getPixel(pixels, i, j, largura));
			k++;
		}
