...
/**
 * Classe que faz a operacao logica NAnd sobre imagens
...
public class Nand {
	...
	/**
	 * Metodo nand que faz a operacao nand com varias imagens gerando uma outra com o resultado
	 ...
	 */	
	public static Image nand(List<ImageIcon> imagensIcon) throws OperacaoLogicaException{
		imageResultHeight = 0;
		imageResultWidth = 0;
		int altura, largura;
		...
		while (k < imagensIcon.size()) {
			...
			for (int i = 0; i < altura; i++) 
				for (int j = 0; j < largura; j++) 
					resultImage[PixelUtils.getIndex(i, j, imageResultWidth)] =  PixelUtils.nand(
							resultImage[PixelUtils.getIndex(i, j, imageResultWidth)],PixelUtils.getPixel(pixels, i, j, largura));
			k++;
		}
