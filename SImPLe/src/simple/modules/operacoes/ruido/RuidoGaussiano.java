package simple.modules.operacoes.ruido;
...
public class RuidoGaussiano extends Thread{
	...
	public RuidoGaussiano(Image imagem){
		image = EscalaCinza.nivelCinza(new ImageIcon(imagem));
		mod = new Modificador(image);
		src_1d = mod.getPixels(mod.getRed(), mod.getGreen(), mod.getBlue(), mod.getAlpha());
		d_w = mod.getLargura();
		d_h = mod.getAltura();
	}
	...
	public Image ShotFull(int [] src_1d, int width, int height, float par){
		d_w = width;
		d_h = height;
		...
		return mod.recopoeImagem(dest_1d,d_w,d_h);
	}
	...
	public Image ShotPartial(int [] src_1d, int width, int height, float par){
		d_w = width;
		d_h = height;
		dest_1d = new int[d_w * d_h];
		float ran1;
	...
	public Image Gaussian(double par){
		dest_1d = new int[d_w * d_h];
		double ran1;    
		int src_rgb;
		int result = 0;
