...
/**
 * Constrói o menu que realiza todas as operações locais em uma imagem.
 * Apresenta métodos que contém todas as funcionalidades providas por estes
 * menus.
 */
public class MenuOperacoesLocais extends SimpleMenu {
	...
	private JMenu locais, filtros, ruidos, espaciais, passaBaixa, passaAlta, frequencia, filtroMorfologico, abertura, fechamento, geradorRuido,
			homomorfico, gaussianoFourier, butterworth, filtroIdeal;
	...
	public void media() {
		...
		if (j.getMaskara() > 0) {
			try {
				myImage = new MyImage(getSimple().getFacade().mean(getSimple().getImagefromFrame(), j.getMaskara(), j.getMaskara()));
